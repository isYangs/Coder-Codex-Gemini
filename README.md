...
让 **Claude/Sisyphus** 作为**规划者**调度 **Coder** 执行代码任务、**Codex** 审核代码质量，**Gemini** 提供专家咨询，<br>形成**自动化的多方协作闭环**。
...
|------|-------------------|---------------------------|
| **规划者** | Claude | Sisyphus (Claude Opus) |
| **工具调用** | MCP 协议 | 子代理委托 |
...
*   **Claude**: 👑 **规划者 / 快检者 / 协调者**
    *   负责需求分析、任务拆解、Prompt 优化、快速验收以及最终决策。**严禁直接修改代码/文档。**
*   **Coder**: 🔨 **执行者**
...
flowchart TB
    ...
    subgraph ClaudeLayer ["Claude - 规划者"]
        Claude["🧠 需求分析 & 任务拆解"]
        Prompt["📝 构造精确 Prompt"]
        Review["🔍 结果审查 & 决策"]
    end
...
3. 调用 coder (或 gemini) 工具 → 执行代码生成/修改
       ↓
4. Claude 快速验收，通过后**必须**调用 Codex 审核
       ↓
5. 调用 codex 工具 → **必须**独立 Code Review
       ↓
6. 根据审核结论：✅ 通过 → 继续；⚠️/❌ 有问题 → 委托 Coder 修复 → 再审 → 循环直至通过
```
...
|----------|---------------|------|------|
| **规划者** | Sisyphus | Claude Opus 4.5 | 需求分析、任务拆解、最终决策 |
